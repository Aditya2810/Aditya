...
# load data
Indicators <- read.csv("D:/R files/Indicators.csv")

#Subset Data
count<-read.csv("Countries.csv")
Indicators <- Indicators%>%anti_join(count)
...
# K-means clustering
set.seed(42)
indic_cluster <- kmeans(indic.norm, 2)
indic_cluster$size
...
developed_countries <- subset(indic.norm, Cluster_No == 2, select = CountryName)
